...
export function canSendEmail(email) {
  return getRemainingAttempts(email) > 0;
}

// Record a new submission
export function recordSubmission(email) {
  const submissions = readSubmissions();
  const now = Date.now();
  ...
  writeSubmissions(submissions);
  console.log(`Recorded submission for ${email}. Total in 24h: ${submissions[email].timestamps.length}`);
}
