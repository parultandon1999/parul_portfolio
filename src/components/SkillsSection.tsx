    ...
    <section id="skills" className="py-24 relative">
      ...
      <div className="container mx-auto px-6 lg:px-20 relative z-10">
        <div className="mb-16">
          ...
          <div className="w-96 h-px bg-border" />
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillCategories.map((category, categoryIndex) => (
            <div
              key={category.title}
              className="bg-card border border-border rounded-lg p-6 card-shadow hover:border-foreground/30 transition-all duration-300 animate-fade-in-up opacity-0"
        ...
        <div className="mt-12 flex flex-wrap gap-3 justify-center">
          {['Deep Learning', 'NLP', 'Computer Vision', 'Time Series', 'A/B Testing', 'Feature Engineering', 'MLOps', 'Docker', 'AWS', 'GCP'].map((skill) => (
            <span
              key={skill}
              className="px-4 py-2 bg-secondary text-muted-foreground text-sm font-mono rounded-full border border-border hover:border-foreground/30 hover:text-foreground transition-all cursor-default"
            >
              {skill}
            </span>
          ))}
